...
func _card_darg_input(event: InputEvent) -> void:
	...
	if (event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT) or is_touch_event:
		if event.is_pressed():
			...
			is_dragging = false
			cur_card.z_index -= 1
			cur_card_pos = Vector2.ZERO
			cur_card._check_boundary()
			cur_card = null
			get_viewport().set_input_as_handled()
		...
		var target_pos = cur_card_pos - drag_offset
		var move_pos = get_global_transform().affine_inverse() * target_pos
		if cur_card:
			cur_card.set_move_pos(target_pos)
			cur_card._check_boundary()
		print("drag card box")
